...
(function ($) {
    CKBox.open = function (options) {
        ...
        if (options.handler == 'inline' && options.content && $('.ckboxmodal #' + options.content).length) {
            ...
            var styles = '';
            if (options.size.x) {
                styles += 'width:' + options.size.x + ';';
            }
            if (options.size.y) {
                styles += 'height:' + (parseInt(options.size.y) + 50) + 'px;';
            }
            ...
            if (!$('.ckboxmodal-back').length) {
                $(document.body).append('<div class="ckboxmodal-back" onclick="CKBox.close()"/>');
            }
//	}
            ckboxmodal = $('#' + boxid);
