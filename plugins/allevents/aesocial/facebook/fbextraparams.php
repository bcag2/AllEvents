...
if (!class_exists('Facebook', false)) {
    require_once(JPATH_ROOT . '/plugins/allevents/aesocial/facebook/facebook.php');
}
...
class JFormFieldfbextraparams extends JFormField
{
    ...
    /**
     * JFormFieldfbextraparams::getControlGroup()
     *
     * @return mixed|string
     * @throws Exception
     */
    public function getControlGroup()
    {
        ...
        if (($fb_app_id != '') && ($fb_secret_key != '')) {
            ...
            $facebook = new Facebook(['appId' => $fb_app_id, 'secret' => $fb_secret_key]);
            //$user = $facebook->getUser();

            $uri = JUri::getInstance();
            $url = urlencode($uri->current() . '?' . $uri->getQuery());
            $loginUrl = "https://www.facebook.com/dialog/oauth?client_id=" . $fb_app_id . "&redirect_uri=" . $url . "&scope=publish_pages,manage_pages,publish_actions,user_managed_groups,user_groups";
            ...
            if ($user) { //access token ok
                ...
                $groups = $facebook->api('/' . $fb_admin . '/groups/', 'GET', ['access_token' => $fb_token]);
                ...
                $pages = $facebook->api('/' . $fb_admin . '/accounts/', 'GET', ['access_token' => $fb_token]);
                ...
                if ($code && !empty($code)) {
                    ...
                    if (empty($response) || preg_match('/error/i', $response)) {
                        $app->enqueueMessage('Error on access token:<pre>' . $response . '</pre>', 'error');
                    } else {
                        $params = null;
    ...
    /**
     * JFormFieldfbextraparams::getHTLMtextfield()
     *
     * @param mixed $label
     * @param mixed $value
     * @param mixed $name
    ...
    /**
     * JFormFieldfbextraparams::getHTLMmsgfield()
     *
     * @param mixed $text
     * @param mixed $class
     *
    ...
    /**
     * JFormFieldfbextraparams::getHTLMcheckboxfield()
     *
     * @param mixed $label
     * @param mixed $value
     * @param mixed $name
    ...
    /**
     * JFormFieldfbextraparams::get_url_contents()
     *
     * @param mixed $url
     *
     * @return mixed
    ...
    /**
     * JFormFieldfbextraparams::getInput()
     *
     * @return null|string
     * @throws Exception
     */
    protected function getInput()
    {
        ...
        if (($fb_app_id != '') && ($fb_secret_key != '')) {
            ...
            echo "<li><input type='hidden' name='jform[params][" . $this->fieldname . "][fb_ids]'   id='jform_params_fbextraparams_fb_ids'   value='" . [] . "'/></li>";
        }
        echo "</ul>";
        echo "</div>";
        echo "<style>  .controls #fb_extra_params {margin-left: -160px;} #fb_extra_params li,#fb_extra_params ul{list-style: none none;margin:0;padding:0;}
                 #fb_extra_params input {float: left;margin: 2px 5px 5px 0;width: auto;padding: 2px;}
                 #fb_extra_params label {line-height: 2em;clear: left;min-width: 12em;float: left;margin:5px 5px 10px 0; font-weight: bold;padding: 0;border: 0;background: transparent;} .fb_bold { font-weight:bold; color:#686;} .fb_label{font-size:11px;width:145px;} 
                 div.fb_box label.fb_message { font-weight:bold; color:#600; width: auto; background-color: #FEE; padding: 5px 8px; border: 1px solid #F99; float: none; margin: 5px auto;}
