...
$sReturn = '<ul class="mod_aelist9-items ' . $moduleclass_sfx . '">';
...
if (!empty($items)) {
    foreach ($items as $obj) {
        ...
        if ($obj->start_month > 0) {
            $sReturn .= '<li style="background-color: ' . $couleur_fond . ';" class="mod_aelist9-item">';
            $sReturn .= '    <div style="background-color: ' . $couleur_texte . ';color: ' . $couleur_fond . ';" class="mod_aelist9-badge">';
            $sReturn .= '        <span class="mod_aelist9-day">' . $obj->start_day . '</span>';
            $sReturn .= '        <span class="mod_aelist9-month">' . $arrMonthNames[$obj->start_month - 1] . '</span>';
            $sReturn .= '        <span class="mod_aelist9-year">' . $obj->start_year . '</span>';
            $sReturn .= '    </div>';
            $sReturn .= '    <div style="color: ' . $couleur_texte . ';" class="mod_aelist9-description">';
            $sReturn .= '        <a style="color: ' . $couleur_texte . ';" href="' . $link . '" class="mod_aelist9-title">' . $obj->titre . '</a>';
            if ($obj->allday == 0) {
                $sReturn .= '<span style="color: ' . $couleur_texte . ';" class="mod_aelist9-time">';
            ...
            $sReturn .= '        <p style="color: ' . $couleur_texte . ';" class="mod_aelist9-details">' . $obj->intro . '</p>';
            $sReturn .= '    </div>';
            $sReturn .= '</li>';
        }
    }
