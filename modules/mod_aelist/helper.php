 ...
 * @access    public
 */
abstract class modAEListHelper
{
    /**
     * modAEListHelper::getItems()
     *
     * @param $params
     *
     * @return mixed
    ...
    public static function getItems($params)
    {
        ...
        $from_days = (int)$params->get('from_days', -3600);
        $specific_date = $params->get('specific_date');
        $specific_days = (int)$params->get('specific_days');
        ...
        if ($myevents > 0) {
            $query->where('(a.proposed_by = ' . $user->get('id') . ')');
        }

        if ($myenrolments > -1) {
            if ($myenrolments == 0) {
                $query->innerJoin('`#__allevents_enrolments` AS e ON e.event_id = a.id and e.enroltype = 0 and user_id =' . $user->get('id'));
            } elseif ($myenrolments == 1) {
                $query->innerJoin('`#__allevents_enrolments` AS e ON e.event_id = a.id and e.enroltype = 1 and user_id =' . $user->get('id'));
            } elseif ($myenrolments == 2) {
                $query->innerJoin('`#__allevents_enrolments` AS e ON e.event_id = a.id and e.enroltype = 2 and user_id =' . $user->get('id'));
            }
        }
    ...
    /**
     * modAEListHelper::getEntityTitle()
     *
     * @param mixed $obj
     * @param mixed $dc
     *
