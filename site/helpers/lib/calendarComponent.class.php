...
class calendarComponent extends iCalBase
{
     ...
     * @param bool|int $propix specific property in case of multiply occurences
     *
    ...
    function _notExistProp($propName)
    {
        ...
        if ('last-modified' == $propName) {
            if (!isset($this->lastmodified)) return true;
        } elseif ('percent-complete' == $propName) {
            if (!isset($this->percentcomplete)) return true;
        } elseif ('recurrence-id' == $propName) {
            if (!isset($this->recurrenceid)) return true;
        } elseif ('related-to' == $propName) {
            if (!isset($this->relatedto)) return true;
        } elseif ('request-status' == $propName) {
            if (!isset($this->requeststatus)) return true;
    ...
    function getProperty($propName = false, $propix = false, $inclParam = false, $specform = false)
    {
        ...
        switch ($propName) {
            ...
            case 'DURATION':
                ...
                return ($inclParam) ? ['value' => $value, 'params' => $params] : $value;
                break;
            case 'EXDATE':
     ...
     * @return bool
     * @internal param mixed $args variable number of function arguments,
    ...
    function setProperty()
    {
        ...
        switch ($arglist[0]) {
            ...
            case 'TRIGGER':
                return $this->setTrigger($arglist[1], $arglist[2], $arglist[3], $arglist[4], $arglist[5], $arglist[6], $arglist[7], $arglist[8], $arglist[9], $arglist[10]);
            case 'TZID':
    ...
    {
        if (empty($value)) if ($this->getConfig('allowEmpty')) $value = '';
        else return false;
    ...
    {
        if (empty($value)) {
            if ($this->getConfig('allowEmpty')) $value = '';
            else return false;
        }
    ...
    function setDuration($week, $day = false, $hour = false, $min = false, $sec = false, $params = false)
    {
        ...
        elseif (is_string($week) && (3 <= strlen(trim($week)))) {
            ...
            $this->duration = ['value' => iCalUtilityFunctions::_duration2arr(['week' => $week, 'day' => $day, 'hour' => $hour, 'min' => $min, 'sec' => $sec])
    ...
    {
        if (empty($exruleset)) if ($this->getConfig('allowEmpty')) $exruleset = '';
        else return false;
     ...
     * @param bool|int $day optional
     * @param bool|int $hour optional
     * @param bool|int $min optional
     * @param bool|int $sec optional
     * @param array|bool $params optional
     *
     * @return bool
     * @uses   calendarComponent::$lastmodified
    ...
    {
        if (empty($value) && !is_numeric($value)) if ($this->getConfig('allowEmpty')) $value = '';
        else return false;
    ...
    {
        if (empty($statcode) || empty($text)) if ($this->getConfig('allowEmpty')) $statcode = $text = '';
        else return false;
    ...
    {
        if (empty($rruleset)) if ($this->getConfig('allowEmpty')) $rruleset = '';
        else return false;
