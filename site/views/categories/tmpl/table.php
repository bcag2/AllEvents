...
?>
    ...
    <div id="allevents_categories" class="row-fluid">
        <?php $show = false; ?>
        <?php foreach ($this->items as $item) : ?>
            ...
            <div class="item column-1 span3">
                <a href="<?php echo JRoute::_('index.php?option=com_allevents&view=events&ct=' . json_encode($ct)); ?>"><img
                        alt="<?php echo $item->titre; ?>" src="<?php echo $item->vignette; ?>" class="thumbnail"></a>
                <div></div>
                <div>
                    <a href="<?php echo JRoute::_('index.php?option=com_allevents&view=events&ct=' . json_encode($ct)); ?>"><?php echo $item->titre; ?></a>&nbsp;(<?php echo($item->nb_events); ?>
                    )
                </div>
            </div>
        <?php endforeach; ?>
