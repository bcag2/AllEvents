 ...
 * @access    public
 */
class AllEventsModelRessource extends JModelAdmin
{
    /**
     *
    ...
    /**
     * AllEventsModelRessource::getForm()
     *
     * @param array $data
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_allevents.ressource', 'ressource', ['control' => 'jform', 'load_data' => $loadData]);
    ...
    /**
     * AllEventsModelRessource::validate()
     *
     * @param mixed $form
     * @param mixed $data
     * @param mixed $group
    ...
    {
        $id = (!empty($data['id'])) ? $data['id'] : (int)$this->getState('ressource.id');
    ...
    /**
     * AllEventsModelRessource::featured()
     *
     * @param mixed $cid
     *
     * @return bool
    ...
    public function featured($cid)
    {
        ...
        if (count($cid)) {
            ...
            $cids = implode(',', $cid);
            $query = 'UPDATE `#__allevents_ressources` SET `default` = 1 WHERE id IN ( ' . $cids . ' )';
            ...
            $query = 'UPDATE `#__allevents_ressources` SET `default` = 0 WHERE id NOT IN ( ' . $cids . ' )';
            $this->_db->setQuery($query);
            try {
                $this->_db->execute();
            } catch (Exception $e) {
    ...
    /**
     * AllEventsModelRessource::unfeatured()
     *
     * @param mixed $cid
     *
     * @return bool
    ...
    public function unfeatured($cid)
    {
        if (count($cid)) {
            ArrayHelper::toInteger($cid);
            $cids = implode(',', $cid);
            $query = 'UPDATE `#__allevents_ressources` SET `default` = 0 WHERE id IN ( ' . $cids . ' )';
            try {
                $this->_db->setQuery($query);
                $this->_db->execute();
            } catch (Exception $e) {
    ...
    /**
     * AllEventsModelRessource::duplicate()
     *
     * @param mixed $pks
     *
     * @return bool
    ...
    /**
     * AllEventsModelRessource::getTable()
     *
     * @param string $type
     * @param string $prefix
     * @param array $config
     ...
     * @return bool|JTable
     */
    public function getTable($type = 'Ressource', $prefix = 'AllEventsTable', $config = [])
    {
    ...
    /**
     * AllEventsModelRessource::loadFormData()
     *
     * @return array|mixed
     * @throws Exception
     */
    protected function loadFormData()
    {
        ...
        $data = JFactory::getApplication()->getUserState('com_allevents.edit.ressource.data', []);
    ...
    /**
     * AllEventsModelRessource::getItem()
     *
     * @param mixed $pk
     *
     * @return mixed
    ...
    /**
     * AllEventsModelRessource::prepareTable()
     *
     * @param mixed $table
     */
    protected function prepareTable($table)
    {
        ...
        if (empty($table->id)) {
            ...
            if (@$table->ordering === '') {
                $db = $this->getDbo();
                $db->setQuery('SELECT MAX(ordering) FROM #__allevents_ressources');
                $max = $db->loadResult();
