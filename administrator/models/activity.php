 ...
 * @access    public
 */
class AllEventsModelActivity extends JModelAdmin
{
    protected $text_prefix = 'COM_ALLEVENTS';

    /**
     * AllEventsModelActivity::getForm()
     *
     * @param array $data
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_allevents.activity', 'activity', ['control' => 'jform', 'load_data' => $loadData]);
    ...
    /**
     * AllEventsModelActivity::validate()
     *
     * @param mixed $form
     * @param mixed $data
     * @param mixed $group
    ...
    public function validate($form, $data, $group = null)
    {
        ...
        if ($id == 0) {
            ...
            if (empty($data['vignette'])) {
                $data['vignette'] = 'images/com_allevents/bullets/activity.png';
            }
        }
        if ($data['alias'] == "") {
            $data['alias'] = JFilterOutput::stringURLSafe($data['titre']);
    ...
    /**
     * AllEventsModelActivity::featured()
     *
     * @param mixed $cid
     *
     * @return bool
    ...
    public function featured($cid)
    {
        if (count($cid)) {
            ...
            $cids = implode(',', $cid);
            $query = 'UPDATE `#__allevents_activities` SET `default` = 1 WHERE id IN ( ' . $cids . ' )';
            ...
            $query = 'UPDATE `#__allevents_activities` SET `default` = 0 WHERE id NOT IN ( ' . $cids . ' )';
            $this->_db->setQuery($query);
            try {
                $this->_db->execute();
            } catch (Exception $e) {
    ...
    /**
     * AllEventsModelActivity::unfeatured()
     *
     * @param mixed $cid
     *
     * @return bool
    ...
    public function unfeatured($cid)
    {
        if (count($cid)) {
            ArrayHelper::toInteger($cid);
            $cids = implode(',', $cid);
            $query = 'UPDATE `#__allevents_activities` SET `default` = 0 WHERE id IN ( ' . $cids . ' )';
            try {
                $this->_db->setQuery($query);
                $this->_db->execute();
            } catch (Exception $e) {
    ...
    /**
     * AllEventsModelActivity::duplicate()
     *
     * @param mixed $pks
     *
     * @return bool
    ...
    /**
     * AllEventsModelActivity::getTable()
     *
     * @param string $type
     * @param string $prefix
     * @param array $config
     ...
     * @return bool|JTable
     */
    public function getTable($type = 'Activity', $prefix = 'AllEventsTable', $config = [])
    {
    ...
    /**
     * AllEventsModelActivity::loadFormData()
     *
     * @return array|mixed
     * @throws Exception
     */
    protected function loadFormData()
    {
        ...
        $data = JFactory::getApplication()->getUserState('com_allevents.edit.activity.data', []);
    ...
    /**
     * AllEventsModelActivity::getItem()
     *
     * @param mixed $pk
     *
     * @return mixed
    ...
    /**
     * AllEventsModelActivity::prepareTable()
     *
     * @param mixed $table
     */
    protected function prepareTable($table)
    {
        ...
        if (empty($table->id)) {
            ...
            if (@$table->ordering === '') {
                $db = $this->getDbo();
                $db->setQuery('SELECT MAX(ordering) FROM #__allevents_activities');
                $max = $db->loadResult();
