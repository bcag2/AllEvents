...
class AllEventsViewRessource extends JViewLegacy
{
    ...
    /**
     * AllEventsViewRessource::addToolbar()
     *
     */
    protected function addToolbar()
    {
        ...
        JToolbarHelper::title(JText::_('RESOURCE'), 'ressource.png');
        // If not checked out, can save the item.
        if (!$checkedOut && ($canDo->get('core.edit') || ($canDo->get('core.create')))) {
            JToolbarHelper::apply('ressource.apply', 'JTOOLBAR_APPLY');
            JToolbarHelper::save('ressource.save', 'JTOOLBAR_SAVE');
        }
        if (!$checkedOut && ($canDo->get('core.create'))) {
            JToolbarHelper::custom('ressource.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);
        }
        // If an existing item, can save to a copy.
        if (!$isNew && $canDo->get('core.create')) {
            JToolbarHelper::custom('ressource.save2copy', 'save-copy.png', 'save-copy_f2.png', 'JTOOLBAR_SAVE_AS_COPY', false);
        }
        if (empty($this->item->id)) {
            JToolbarHelper::cancel('ressource.cancel', 'JTOOLBAR_CANCEL');
        } else {
            JToolbarHelper::cancel('ressource.cancel', 'JTOOLBAR_CLOSE');
        }
    }
}