...
class AllEventsHelperIcal
{
    ...
    public static function createIcalFromEvents($events, $asDownload = false)
    {
        ...
        $text[] = 'VERSION:2.0';
        $text[] = 'PRODID:ALLEVENTS';
        $text[] = 'CALSCALE:GREGORIAN';

        $userTz = AllEventsHelper::getDate()->getTimezone()->getName();
        if (empty($userTz)) {
            $userTz = 'UTC';
        }
        ...
        foreach ($events as $key => $event) {
            ...
            if (!empty($event->category_title)) {
                ...
                $calendars[$event->catid] = AllEventsHelper::getCalendar($event->catid)->title;
            }
        }
        // $text[] = 'X-WR-CALNAME:'.implode('; ', $calendars);

        $now = AllEventsHelper::getDate()->format('Ymd\THis\Z');
        foreach ($events as $key => $event) {
        ...
        if ($asDownload) {
            ...
            header('Content-disposition: attachment; filename="' . AllEventsHelper::getDate()->format('YmdHis') . '.ics"');
    ...
    private static function addEventData($event)
    {
        ...
        $start = AllEventsHelper::getDate($event->start_date, $event->all_day);
        $end = AllEventsHelper::getDate($event->end_date, $event->all_day);
        ...
        if ($event->all_day == 1) {
            ...
            $tmp = AllEventsHelper::getDate($event->end_date, $event->all_day);
            $tmp->modify('+1 day');
        ...
        if (!empty($event->rrule)) {
            ...
            if ($event->id && is_numeric($event->catid)) {
                ...
                $cal->expand($start, new DateTime('2038-01-01'));

                // Find deleted events
                $exdates = [];
                foreach ($cal->VEVENT as $vevent) {
                    $found = false;
                    $date = AllEventsHelper::getDate($vevent->DTSTART->getDateTime()->format('U'), $event->all_day);
        ...
        if (isset($event->uid)) {
            ...
            $text[] = 'UID:' . md5($event->original_id . '_AllEvents');
        } else if (!empty($event->id)) {
            $text[] = 'UID:' . md5($event->id . '_AllEvents');
        } else {
            $text[] = 'UID:' . md5(uniqid() . '_AllEvents');
        }
        ...
        $text[] = 'DTSTAMP:' . AllEventsHelper::getDate($event->created)->format('Ymd\THis\Z');
        ...
        if (isset($event->locations) && !empty($event->locations)) {
            $text[] = 'LOCATION:' . self::icalEncode(AllEventsHelperLocation::format($event->locations));
        ...
        if ($event->modified && $event->modified != JFactory::getDbo()->getNullDate()) {
            $text[] = 'LAST-MODIFIED:' . AllEventsHelper::getDate($event->modified)->format('Ymd\THis\Z');
        }
