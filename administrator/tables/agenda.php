 ...
 * @access    public
 */
class AllEventsTableagenda extends JTable
{
    /**
     * Constructor
     *
    ...
    public function bind($array, $ignore = '')
    {
        ...
        if (($task == 'save' || $task == 'apply') && (!JFactory::getUser()->authorise('core.satellites', 'com_allevents') && $array['published'] == 1)) {
            $array['published'] = 0;
        }
        ...
        if (!JFactory::getUser()->authorise('core.satellites', 'com_allevents')) {
            $actions = JAccess::getActionsFromFile(JPATH_SITE . '/administrator/components/com_allevents/access.xml', "/access/section[@name='agenda']/");
            $default_actions = JAccess::getAssetRules('com_allevents.agenda.' . $array['id'])->getData();
            $array_jaccess = [];
    ...
    protected function _getAssetName()
    {
        ...
        return 'com_allevents.agenda.' . (int)$this->$k;
    }
